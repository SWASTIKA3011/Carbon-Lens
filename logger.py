...
def log_action(action, code):
    timestamp = datetime.datetime.now().isoformat()
    c.execute("INSERT INTO actions (timestamp, action, user_code) VALUES (?, ?, ?)",
            (timestamp, action, code))
    conn.commit()

def get_user_actions(code):
    c.execute("SELECT timestamp, action FROM actions WHERE user_code=?", (code,))
    return c.fetchall()
...
def get_recent_anonymous_actions(limit=20):
    c.execute("SELECT action, MAX(timestamp) FROM actions GROUP BY action ORDER BY timestamp DESC LIMIT ?", (limit,))
    return c.fetchall()
        ...
        if selected_category == "Invested in Eco Projects":
            ...
            with st.expander("**Want to add extra info?**"):
                ...
                if selected_action == "Invested in Carbon Offsetting Project":
                    ...
                    # impact_factor = st.number_input("**CO2 saved per dollar $ investment?**", 0.0, 100.0, 0.5)
                    notes = st.text_input("**Enter any additional notes (optional):**")
